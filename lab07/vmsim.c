...
int physicalAddress(uint vAddr, char action){
    ...
    int pAddr = 0;
    // if the page number over the total size
    if (pn >= nPages) {
        return -1;
    }
    
    if(PageTable[pn].status != NotLoaded){                  // if it is loaded
        // if some buddy toched set modified
        if(action == 'W'){PageTable[pn].status = Modified;}
        // and he must toched at some time
        PageTable[pn].lastAccessed = clock;
        pAddr = (PAGESIZE * (PageTable[pn].frameNo)) + offset;
    } else if (PageTable[pn].status == NotLoaded){          // if it is not loaded
        int i = 0;
        int frameN = 0;
        int zhishi = 0;
        // find a frame which is not used
        for(i = 0; ((i < nFrames )&&(zhishi != 100)); i++){
            if(MemFrames[i] == -1){
                zhishi = 100;
            }
        }
        ...
        if(i < nFrames){
            ...
            // increase the replaces as required
            nReplaces++;
            int LRU = -1;
            int min = clock;
            int k = 0;
            // find the min and replace the value
            while(k < nPages){
            ...
            // if it is modified
            if(PageTable[LRU].status == Modified){         // increment n writes
                nSaves++;                                   // in writes + 1
            }
        ...
        PageTable[pn].status = (action == 'W')?Modified:Loaded;
        PageTable[pn].lastAccessed = clock;
        PageTable[pn].frameNo = frameN;
        pAddr = (PageTable[pn].frameNo)*PAGESIZE + offset;
        MemFrames[frameN] = pn;
    }
   return pAddr;
}
